...
async def start_download(query, context, codec):
    ...
    # FIX: initialise status_msg to None so the except block always has a reference
    status_msg = None
    ...
    if codec in ['mp3', 'm4a', 'wav', 'flac']:
        ...
        ydl_opts.update({'format': f'bestvideo[height={res}][vcodec^=av01]+bestaudio/best', 'merge_output_format': 'mp4'})

    elif codec == 'vp9':
        status_msg = await query.edit_message_text(f"⏳ Fetching {res}p in VP9...")
        ydl_opts.update({'format': f'bestvideo[height={res}][vcodec^=vp9]+bestaudio/best', 'merge_output_format': 'mp4'})

    elif codec == 'h264_native':
        status_msg = await query.edit_message_text(f"⏳ Fetching {res}p in Native H.264...")
        ydl_opts.update({'format': f'bestvideo[height={res}][vcodec^=avc1]+bestaudio/best', 'merge_output_format': 'mp4'})

    elif codec == 'h264_transcode':
        status_msg = await query.edit_message_text(f"⏳ Fetching {res}p to transcode to H.264...")
        # FIX: removed height filter from audio selector — height has no meaning for audio streams
    ...
    try:
        ...
        if status_msg:
            ...
            await context.bot.send_message(chat_id=query.message.chat_id, text=friendly_error, parse_mode="Markdown")
